...
class RafAnimation {
  ...
  private stepByStep() {
    let current =
      ((window.performance.now() - this.startTime) / this.duration) *
        (this.end - this.start) +
      this.start;
    current = this.flag
      ? Math.min(current, this.end)
      : Math.max(current, this.end);
    this.cb(current);
    if (current === this.end) {
...
export function animate(
  ...
  return ENABLE_RAF
    ? new RafAnimation(start, end, duration, cb, endCb)
    : new TimerAnimation(start, end, duration, cb, endCb);
}
